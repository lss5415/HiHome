...
/**
 * @author LSS 2015年10月19日 下午4:33:47 月任务总览
 */
public class B1_03_YueRenWuZongLan extends BaseActivity implements
		IXListViewListener, OnItemClickListener {
	private static int PERPAGE = 10;// 每页显示的任务数为10
	...
	private String date = "";// 获取时间段任务的时间
	private CommonAdapter<Task> adapter;
	private List<Task> tasks = new ArrayList<Task>();
	private String[] stateStr = new String[] { "未接受", "已接受", "待执行", "执行中",
			"已完成", "已取消" };
	private String[] tipStr = new String[] { "不提醒", "正点", "五分钟", "十分钟", "一小时",
			"一天", "三天" };
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		edate = DateUtil.dateToString(cal.getTime(), "yyyy-MM")
				+ sdate.substring(7);// 下月
		date = sdate + "~" + edate;
		initView();
		requestData();
	}
	...
	private void initView() {
		...
		aci_date.setText(date);
		task_listview = (XListView) findViewById(R.id.aci_listview);// 任务列表

		adapter = new CommonAdapter<Task>(this, R.layout.ui_b1_03_zonglan_item,
				tasks) {
			@Override
			public void convert(ViewHolder holder, Task task) {
				String datastart = StringUtil.isEmpty(task.getStart()) ? "00-00"
						: task.getStart().substring(5, 10);
				String dataend = StringUtil.isEmpty(task.getEnd()) ? "00-00"
						: task.getEnd().substring(5, 10);
				...
				holder.setText(
						R.id.date,
						Html.fromHtml("<font color=#EA5414>" + datastart
								+ "</font><br><small>~" + dataend + "</small>"))
